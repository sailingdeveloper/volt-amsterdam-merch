@props(['product', 'isTopSeller' => false, 'compact' => false])

<div class="group bg-white rounded-xl shadow-sm hover:shadow-lg transition-shadow overflow-hidden">
    <a href="{{ route('products.show', $product->slug) }}" class="block">
        <div class="aspect-square bg-gray-100 relative overflow-hidden">
            @if($product->image)
                <img src="{{ Storage::url($product->image) }}"
                     alt="{{ $product->localized_name }}"
                     class="h-full w-full object-cover group-hover:scale-105 transition-transform duration-300">
            @else
                <div class="h-full w-full flex items-center justify-center bg-gradient-to-br from-volt-purple/10 to-volt-purple/20">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-16 h-16 text-volt-purple/40">
                        <path stroke-linecap="round" stroke-linejoin="round" d="m2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 0 0 1.5-1.5V6a1.5 1.5 0 0 0-1.5-1.5H3.75A1.5 1.5 0 0 0 2.25 6v12a1.5 1.5 0 0 0 1.5 1.5Zm10.5-11.25h.008v.008h-.008V8.25Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" />
                    </svg>
                </div>
            @endif

            @if($isTopSeller)
                <div class="absolute top-2 left-2">
                    <span class="bg-volt-yellow text-volt-purple px-2 py-1 rounded-full text-xs font-bold shadow">
                        {{ __('shop.favorite') }}
                    </span>
                </div>
            @endif

            @if(!$product->isInStock())
                <div class="absolute inset-0 bg-black/50 flex items-center justify-center">
                    <span class="bg-red-500 text-white px-4 py-2 rounded-full text-sm font-medium">
                        {{ __('shop.out_of_stock') }}
                    </span>
                </div>
            @endif
        </div>

        <div class="p-3 sm:p-4">
            <h3 class="font-semibold text-sm sm:text-base text-gray-900 group-hover:text-volt-purple transition-colors line-clamp-2">
                {{ $product->localized_name }}
            </h3>
            <p class="mt-1 text-base sm:text-lg font-bold text-volt-purple">
                &euro;{{ $product->formatted_price }}
            </p>
        </div>
    </a>

    <div class="px-3 pb-3 sm:px-4 sm:pb-4">
        @if($product->isOrderable())
            <form action="{{ route('cart.add') }}" method="POST" data-add-to-cart>
                @csrf
                <input type="hidden" name="product_id" value="{{ $product->id }}">
                @if($product->hasSizes())
                    <div class="flex {{ $compact ? 'flex-col' : 'flex-col sm:flex-row' }} gap-2">
                        <select name="size" required
                                class="{{ $compact ? 'w-full' : 'w-full sm:w-20' }} bg-white text-gray-700 text-sm font-medium py-2 px-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-volt-purple focus:border-volt-purple cursor-pointer">
                            <option value="">{{ __('shop.size') }}</option>
                            @foreach($product->ordered_sizes as $size => $stock)
                                @if($stock > 10)
                                    <option value="{{ $size }}">{{ $size }}</option>
                                @elseif($stock > 0)
                                    <option value="{{ $size }}">{{ $size }} ({{ __('shop.x_left', ['count' => $stock]) }})</option>
                                @endif
                            @endforeach
                        </select>
                        <button type="submit"
                                class="flex-1 bg-volt-purple hover:bg-volt-purple-dark text-white text-sm font-medium py-2 px-3 rounded-lg transition-colors whitespace-nowrap inline-flex items-center justify-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" fill="currentColor" class="w-4 h-4 flex-shrink-0">
                                <path d="M24 0C10.7 0 0 10.7 0 24S10.7 48 24 48H76.1l60.3 316.5c2.2 11.3 12.1 19.5 23.6 19.5H488c13.3 0 24-10.7 24-24s-10.7-24-24-24H179.9l-9.1-48h317c14.3 0 26.9-9.5 30.8-23.3l54-192C578.3 52.3 563 32 541.8 32H122l-2.4-12.5C117.4 8.2 107.5 0 96 0H24zM176 512c26.5 0 48-21.5 48-48s-21.5-48-48-48s-48 21.5-48 48s21.5 48 48 48zm336-48c0-26.5-21.5-48-48-48s-48 21.5-48 48s21.5 48 48 48s48-21.5 48-48zM252 160c0-11 9-20 20-20h44V96c0-11 9-20 20-20s20 9 20 20v44h44c11 0 20 9 20 20s-9 20-20 20H356v44c0 11-9 20-20 20s-20-9-20-20V180H272c-11 0-20-9-20-20z"/>
                            </svg>
                            @if(!$compact)<span>{{ __('shop.add_to_cart') }}</span>@endif
                        </button>
                    </div>
                @else
                    <button type="submit"
                            class="w-full bg-volt-purple hover:bg-volt-purple-dark text-white text-sm font-medium py-2 px-3 rounded-lg transition-colors inline-flex items-center justify-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" fill="currentColor" class="w-4 h-4 flex-shrink-0">
                            <path d="M24 0C10.7 0 0 10.7 0 24S10.7 48 24 48H76.1l60.3 316.5c2.2 11.3 12.1 19.5 23.6 19.5H488c13.3 0 24-10.7 24-24s-10.7-24-24-24H179.9l-9.1-48h317c14.3 0 26.9-9.5 30.8-23.3l54-192C578.3 52.3 563 32 541.8 32H122l-2.4-12.5C117.4 8.2 107.5 0 96 0H24zM176 512c26.5 0 48-21.5 48-48s-21.5-48-48-48s-48 21.5-48 48s21.5 48 48 48zm336-48c0-26.5-21.5-48-48-48s-48 21.5-48 48s21.5 48 48 48s48-21.5 48-48zM252 160c0-11 9-20 20-20h44V96c0-11 9-20 20-20s20 9 20 20v44h44c11 0 20 9 20 20s-9 20-20 20H356v44c0 11-9 20-20 20s-20-9-20-20V180H272c-11 0-20-9-20-20z"/>
                        </svg>
                        @if(!$compact)<span>{{ __('shop.add_to_cart') }}</span>@endif
                    </button>
                @endif
            </form>
        @elseif($product->orderable === false)
            <a href="{{ route('products.show', $product->slug) }}"
               class="block w-full text-center bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium py-2 px-3 rounded-lg transition-colors">
                {{ __('shop.view') }}
            </a>
        @endif
    </div>
</div>
